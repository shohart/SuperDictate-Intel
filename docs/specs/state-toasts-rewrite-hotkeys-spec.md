...
2. **Цветовое кодирование** (акцентная кайма + цвет текста-статуса):
   - коррекция включена → `systemGreen`;
   - выключено → нейтральный серый;
   - **режимы рерайта кодируются индивидуально**, чтобы режим был
     опознаваем с одного взгляда:
     - Причесать (polish) → `systemTeal`;
     - Задача (structuredTask) → `systemOrange`;
     - Официальный (official) → `systemIndigo`;
   - «Рерайт · вкл» окрашивается цветом ТЕКУЩЕГО режима (не зелёным).
...
4a. **Позиционирование** — тот же отработанный механизм, что у тоста
   записи и тоста запоминания замены: `FocusedTextTargetResolver.
   captureTarget()` (read-only AX-захват) + `resolveElementFrame` → тост
   всплывает над полем ввода, где курсор; если текстового поля нет —
   фолбэк в правый нижний угол экрана.
...
| Рерайт включён | `Рерайт · вкл · <стиль>` | `Rewrite · on · <style>` | цвет стиля |
| Рерайт выключен | `Рерайт · выкл` | `Rewrite · off` | off |
| Смена режима | `Режим · <стиль>` | `Style · <style>` | цвет стиля |
