...
// include/ggml-vulkan-shaders-runtime.h. See that header for the full
// design rationale; this file only has the base-path resolution + the
...
// ggml-vulkan-shaders-runtime.cpp) call into.
//
// Deliberately plain C++17 stdlib only (<fstream>/<mutex>/...) — no
// Vulkan, no Foundation/AppKit, no Objective-C++. Path resolution is
// entirely string-based (see ggml_vk_shaders_set_directory's doc comment
// for the 3-tier fallback), matching this codebase's existing convention
// of Swift resolving bundle paths and handing plain C strings down into
// the whisper_cpp C API (WhisperEngine.swift -> modelPath ->
// whisper_init_from_file_with_params) rather than having vendored C++
// code query NSBundle/CFBundle itself.
...
// swift/Sources/whisper_cpp/ggml-vulkan-shaders-runtime.cpp (this file)
// sits directly alongside swift/Sources/whisper_cpp/vulkan-shaders/ in the
// source tree, so deriving the dev-build fallback from this translation
// unit's own __FILE__ (the absolute path on the machine that compiled it)
// finds the real 1785-file corpus without any configuration — used by
// `swift build`/`swift run` before an .app bundle (and therefore a
...
const loaded_shader & load_locked(const std::string & name) {
    ...
    if (!loaded) {
        ...
        // directory means EVERY shader silently resolves to a null
        // pointer + zero length, which flows straight into
        // vk::ShaderModuleCreateInfo for all 1785 shaders with no
        // guarantee Vulkan validation layers are even enabled to catch
        // it (they normally aren't in a shipped app) — so this is the
        // only diagnostic standing between "GPU transcription silently
        // falls back to garbage/crashes" and a log line pointing at
        // exactly which path was tried.
        if (dir.empty()) {
            std::fprintf(stderr,
