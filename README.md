...
## Ускорение GPU (Vulkan) — экспериментально

На Intel Mac с дискретной AMD GPU в меню приложения (в настройках, среди
поведенческих переключателей) есть пункт `Use GPU (Vulkan) — experimental`.
По умолчанию он выключен: распознавание идёт на CPU через BLAS/Accelerate.
При включении whisper.cpp вместо этого использует бэкенд Vulkan (через
статически слинкованный MoltenVK) для этапа кодирования аудио.

Проверено на реальном железе (Xeon E5-2678 v3 + AMD Radeon RX 6600,
macOS 15.7.7) прогоном по 10 повторов на каждую из 3 контрольных
русскоязычных фраз (30 прогонов всего) с GPU-бэкендом:

- **Обнаружена интермиттирующая ошибка распознавания.** 2 прогона из 30 (оба
  на одной и той же фразе) добавили лишний галлюцинированный текст после
  правильной расшифровки — один раз слово «Реклама», другой раз фразу «Звучит
  музыка.». Остальные 28 прогонов (включая все 20 прогонов двух других фраз)
  совпали с эталонной расшифровкой побайтово. Это именно та интермиттирующая
  проблема (не воспроизводится каждый раз, но и не редкость — здесь это ~7%
  прогонов), из-за которой ранее уже был отклонён предыдущий GPU-бэкенд
  (Metal) в этом форке. **Поэтому GPU-ускорение остаётся опциональным,
  выключенным по умолчанию, и не рекомендуется, если важна 100%-ная точность
  транскрипции.**
- GPU-бэкенд реально использует видеокарту: во время прогонов частота ядра
  GPU кратковременно поднималась до ~1,8 ГГц, а показатель «GPU Activity»
  доходил до 69–72% (обороты вентилятора при этом не менялись — рост, судя по
  всему, слишком короткий и/или недостаточный по теплу для срабатывания
  авторегулировки). Пики не были постоянными: whisper.cpp считает декодер на
  CPU (BLAS) и в текущей сборке GPU занят только частью общего времени
  прогона.
...
- По скорости GPU-прогоны были быстрее CPU-прогонов на тех же 3 фразах
  (сравнение на этой же машине, при этом сильно загруженной посторонними
  процессами во время замера): на 16–38% в зависимости от фразы. Абсолютные
  цифры на этой конкретной машине были заметно выше, чем в более ранних,
  менее загруженных замерах этого проекта — ориентируйтесь на относительную,
  а не абсолютную разницу.
