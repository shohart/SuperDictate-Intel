...
## Распознавание речи: Parakeet TDT 0.6B v3
...
среди поведенческих переключателей) есть пункт `Use GPU (Vulkan)` — в этой
версии он ещё не подключён к реальному Vulkan-бэкенду (это отдельный, более
поздний этап миграции на parakeet.cpp); включение переключателя пока не
меняет поведение, распознавание всё равно идёт на CPU. Раздел про
CPU/Vulkan-замеры на реальном железе (Xeon E5-2678 v3 + AMD Radeon RX 6600)
появится здесь после того, как Vulkan-бэкенд будет реализован и измерен.
